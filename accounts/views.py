...
class EduCenterViewSet(ReadOnlyModelViewSet):
    ...
    queryset = (
        EducationCenter.objects.filter(active=True)
        .annotate(
...
class LikeViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        existing_like = Like.objects.filter(
            user=request.user, content_type=content_type, object_id=edu_center.id
        ).first()
        ...
        like = Like.objects.create(user=request.user, content_object=edu_center)
...
class RegisterView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"user": user_data, "access": access_token, "refresh": refresh_token},
